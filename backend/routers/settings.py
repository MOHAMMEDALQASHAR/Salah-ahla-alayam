...
def read_settings(db = Depends(database.get_db)):
    return crud.get_settings(db)

@router.get("/{key}", response_model=schemas.Setting)
def read_setting(key: str, db = Depends(database.get_db)):
...
def set_setting(setting: schemas.SettingCreate, db = Depends(database.get_db), current_admin = Depends(get_current_admin)):
